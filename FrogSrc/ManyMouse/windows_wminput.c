...
//FROG_CHANGE_BEGIN
/// True if the left and right button input should be reversed.
static int reverseButtons = FALSE;
//FROG_CHANGE_END
...
static void queue_from_rawinput(const RAWINPUT *raw)
{
    ...
    #define QUEUE_BUTTON(x) { \
        if (mouse->usButtonFlags & RI_MOUSE_BUTTON_##x##_DOWN) { \
            if((x == 1) && (reverseButtons)) \
               event.item = 1; \
            else if((x == 2) && (reverseButtons)) \
               event.item = 0; \
            else \
               event.item = x-1; \
            event.value = 1; \
...
static int windows_wminput_init(void)
{
    ...
    //FROG_CHANGE_BEGIN
    reverseButtons = GetSystemMetrics(SM_SWAPBUTTON);
    //FROG_CHANGE_END
    ...
    //FROG_CHANGE_BEGIN
    // If this function returns 0, even if it's just because no mice were
    // found, windows_wminput_quit won't be called.  If no mice were found,
    // don't set up the event queue, since it won't be cleaned up.  This can
    // cause problems otherwise.  For example, if 'class_atom' isn't
    // unregistered, the next attempt to register it will fail.
    if (!available_mice)
       return 0;
    //FROG_CHANGE_END
